...
export function AppShell() {
  ...
  const navItems: { id: View; label: string; icon: React.ElementType; show?: boolean; badge?: number }[] = [
    ...
    { id: "clients", label: "Clientes", icon: Users, badge: clients.length > 0 ? undefined : undefined },
        ...
        <div className="flex items-center justify-between h-14 px-4">
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="md:hidden p-1.5 rounded-lg hover:bg-white/10 active:bg-white/15 transition-colors"
          ...
          <div className="flex items-center gap-1.5">
            ...
            {hasNotifications && (
              <Button
                variant="ghost"
                size="icon"
                className="h-8 w-8 text-white/80 hover:text-white hover:bg-white/10 rounded-full relative"
              ...
              <>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-52">
                      ...
                      <div className="px-2 py-1.5 border-b mb-1">
                        ...
                        {user.isPro && (
                          <p className="text-[10px] text-amber-600 flex items-center gap-1 mt-0.5">
                            <Crown className="h-3 w-3" /> Licencia Pro Activa
                          </p>
                        )}
                      </div>
                    )}
                    <DropdownMenuItem onClick={() => setActiveView("dashboard")} className="cursor-pointer">
                    ...
                    {user?.role === "admin" && (
                      <DropdownMenuItem onClick={() => setActiveView("admin")} className="cursor-pointer">
            ...
            {!isAuthenticated && (
              <Button
                size="sm"
                className="h-8 bg-white/20 text-white hover:bg-white/30 text-xs font-semibold rounded-lg btn-wa"
        ...
        <div className="flex items-center gap-1 px-4 overflow-x-auto">
          {navItems.filter(item => item.show !== false).map((item) => (
            <button
              ...
              className={`relative flex items-center gap-1.5 px-3.5 py-2.5 text-sm font-medium rounded-lg transition-all whitespace-nowrap ${
                activeView === item.id
            ...
            >
              ...
              {item.badge && item.badge > 0 && (
                <span className={`ml-1 min-w-[18px] h-[18px] flex items-center justify-center rounded-full text-[9px] font-bold px-1 ${
                  activeView === item.id
                    ? "bg-[#128C7E] text-white dark:bg-[#25D366] dark:text-[#075E54]"
              ...
              {activeView === item.id && (
                <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-5 h-0.5 rounded-full bg-[#25D366] dark:bg-[#25D366]" />
              )}
            </button>
          ))}
        </div>
          ...
          <div className="p-2 space-y-1">
            {navItems.filter(item => item.show !== false).map((item) => (
              <button
                ...
                className={`flex items-center gap-2.5 w-full px-3 py-2.5 text-sm font-medium rounded-lg transition-all ${
                  activeView === item.id
              ...
              >
                ...
                {item.label}
                {item.badge && item.badge > 0 && (
                  <Badge className="ml-auto bg-red-500 text-white text-[9px] h-5 min-w-[20px] px-1.5">
                    {item.badge}
                  </Badge>
                )}
              </button>
        ...
        <div className="flex items-center justify-around h-16">
          {navItems.filter(i => i.show !== false).slice(0, 4).map((item) => (
            ...
            >
              ...
              {activeView === item.id && (
                <span className="absolute -top-px left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full bg-[#25D366]" />
              )}
            </button>
          ))}
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className={`relative flex flex-col items-center justify-center w-full h-full gap-0.5 transition-all ${
            ...
            <DropdownMenuContent side="top" align="center" className="w-52 mb-2">
              <DropdownMenuItem onClick={() => setActiveView("pricing")} className="cursor-pointer">
                <Crown className="mr-2 h-4 w-4 text-amber-500" /> Precios
              </DropdownMenuItem>
