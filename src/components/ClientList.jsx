...
const COMPANY_PHONE = '51933667414'; // Número único de destino
...
export default function ClientList({ clients, setClients, templates, logMessage, isLicensed, user, isApproved }) {
  ...
  const handleDelete = (id) => {
    if(confirm("¿Eliminar cliente?")) setClients(prev => prev.filter(c => c.id !== id));
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="🔍 Buscar por nombre, teléfono, email..." className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-wa-green outline-none" />

      <div className="flex gap-2 overflow-x-auto pb-2">
        {['Todos', 'Nuevo', 'Pendiente', 'VIP'].map(tag => (
          <button key={tag} onClick={() => setFilterTag(tag)} className={`px-4 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors ${filterTag === tag ? 'bg-wa-dark text-white' : 'bg-white text-gray-600 border border-gray-200'}`}>{tag}</button>
        ))}
      </div>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1 min-w-0 mr-3" onClick={() => setActionModal(client)} style={{cursor: 'pointer'}}>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {tags.map(tag => (
                        <span key={tag} className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${tag === 'VIP' ? 'bg-yellow-100 text-yellow-700' : tag === 'Pendiente' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>{tag}</span>
                      ))}
                    </div>
                    ...
                    {client.company && <p className="text-xs text-gray-400 mt-1">🏢 {client.company}</p>}
                    {(client.notes || []).length > 0 && <p className="text-xs text-gray-400 mt-1 truncate">📝 {(client.notes[client.notes.length - 1]?.text || client.notes[client.notes.length - 1])}</p>}
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => { setEditClient(client); setShowForm(true); }} className="bg-gray-100 text-gray-500 p-2 rounded-lg hover:bg-gray-200 transition-colors">✏️</button>
                    <button onClick={() => handleDelete(client.id)} className="bg-gray-100 text-red-400 p-2 rounded-lg hover:bg-red-50 transition-colors">🗑️</button>
                  </div>
                </div>
              </div>
            );
              ...
              {templates.map(tmpl => {
                ...
                return (
                  <button key={tmpl.id} onClick={() => {
                    ...
                    {tmpl.usageCount > 0 && <p className="text-xs text-gray-400 mt-1">Usada {tmpl.usageCount} veces</p>}
                  </button>
                );
              })}
            </div>
