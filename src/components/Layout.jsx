...
export default function Layout({ children, activeView, setActiveView, isLicensed, onLogin, onLogout, user, showLoginButton = true, clients = [], isApproved = false }) {
    ...
    <div className="flex items-center gap-2">
      ...
      {user && (
        <div className="flex items-center gap-1">
          <span className="text-xs text-wa-light hidden sm:inline truncate max-w-[100px]">{user.email}</span>
          {isApproved && <span className="text-xs">✅</span>}
          {!isApproved && <span className="text-xs">⏳</span>}
        </div>
      )}
      ...
      {user && (
        <button onClick={onLogout} className="bg-red-500/80 px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-red-500 transition-colors">🚪 Salir</button>
      )}
